...
def is_ubiquity(device):
    try:
        return (
            device["identification"]["vendor"] == "Ubiquiti"
            or "lbe" in device["identification"]["name"].lower()
        )
    except:
        return False
...
def generate_uisp_section(devices):
    ...
    if devices is None or len(devices) == 0:
        raise ValueError(
            "Devices none or zero length but at least one is required for generate_uisp_section"
        )
...
def get_report(nn):
    ...
    if devices is None or len(devices) == 0:
        ...
        command = ["nn_stats.sh", str(nn)]
        report += run_nn_stats(nn)
        return report
    elif is_lbe_only(devices):
        lbe_ip = cidr_to_ip(devices[0]["ipAddress"])

        report += (
            f"NN {nn} appears to be an LBE only site, some details will be omitted.\n\n"
        )
